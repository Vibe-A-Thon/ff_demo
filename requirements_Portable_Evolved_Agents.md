...
`FF_PORTABLE_EVOLUTION_PACK.pep.zip`
...
- Import `.pep.zip` OR individual `.amc/.rsb/.brc`
- Pre-import checks:
...
### 9.1 Core services
- `packaging_service`: build/verify `.pep.zip`
...
- Bank A can export `.pep.zip` containing selected AMC/RSB/BRC plus contracts + manifest.
